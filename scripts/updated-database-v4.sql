...
BEGIN
    IF EXISTS (
        ...
        ALTER TABLE talents DROP COLUMN age;
    END IF;
    ...
    IF EXISTS (
        ...
        ALTER TABLE talents DROP COLUMN agency;
    END IF;
END $$;

-- 新しいタレントデータを挿入（年齢と職種を削除）
INSERT INTO talents (name, avatar_url, weekly_topic) VALUES
('中塚智', NULL, '新曲「夢の先へ」リリース決定'),
('五十嵐諒', NULL, 'ファッション誌の表紙に初登場'),
('吉川慶', NULL, 'ドラマ「青い空」出演決定'),
('島田和奏', '/images/shimada-wakana.jpg', '初のソロコンサート開催発表'),
('吉富千桜', NULL, '新CM契約獲得'),
('谷口彩菜', NULL, '海外ファッションショー出演決定');
...
BEGIN
    ...
    -- 中塚智
    (1, 'instagram', 5611, 0, current_week, current_year),
    (1, 'tiktok', 120000, 0, current_week, current_year),
    (1, 'twitter', 1308, 0, current_week, current_year),
    -- 五十嵐諒
    (2, 'instagram', 513, 0, current_week, current_year),
    (2, 'tiktok', 85000, 0, current_week, current_year),
    (2, 'twitter', 540, 0, current_week, current_year),
    -- 吉川慶
    ...
    -- 島田和奏
    (4, 'instagram', 4651, 0, current_week, current_year),
    (4, 'tiktok', 95000, 0, current_week, current_year),
    (4, 'twitter', 937, 0, current_week, current_year),
    -- 吉富千桜
    (5, 'instagram', 59, 0, current_week, current_year),
    (5, 'tiktok', 88000, 0, current_week, current_year),
    (5, 'twitter', 383, 0, current_week, current_year),
    -- 谷口彩菜
